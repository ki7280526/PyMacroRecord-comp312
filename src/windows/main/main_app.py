...
if platform.lower() == "win32":
    from tkinter.ttk import Style, Frame as TTKFrame, Label as TTKLabel
...
class MainApp(Window):
    ...
    def __init__(self):
        ...
        if len(argv) > 1:
            ...
            # Play Button frame and button
            self.playFrame = Frame(self.center_frame)
            self.playFrame.pack(side=LEFT, padx=50)
            self.playBtn = Button(self.playFrame, image=self.playImg, command=self.macro.start_playback)
            self.playBtn.pack()
            ...
            self.playFrame = Frame(self.center_frame)
            self.playFrame.pack(side=LEFT, padx=50)
            self.playBtn = Button(self.playFrame, image=self.playImg, state=DISABLED)
            self.playBtn.pack()

        # Record Button
        self.recordImg = PhotoImage(file=resource_path(path.join("assets", "button", "record.png")))
        # Record Button frame and button  
        self.recordFrame = Frame(self.center_frame)
        self.recordFrame.pack(side=RIGHT, padx=50)
        self.recordBtn = Button(self.recordFrame, image=self.recordImg, command=self.macro.start_record)
        self.recordBtn.pack()
    ...
    def apply_theme(self):
        ...
        # Button frames
        try:
            self.playFrame.config(bg=theme["button_bg"])
            self.recordFrame.config(bg=theme["button_bg"])
        except:
            pass
        ...
        # Try to style the actual buttons with tkinter (not ttk)
        try:
            self.playBtn.config(bg=theme["button_bg"], activebackground=theme["button_bg"])
            self.recordBtn.config(bg=theme["button_bg"], activebackground=theme["button_bg"])
        except:
            pass
